...
import { Package, Users, LogOut, Clock, Plane, Eye } from "lucide-react";
    ...
    async function fetchOrders() {
      try {
        ...
        const data = snapshot.docs
                        ...
                        .filter(order => order.type === "Encomenda" && order.status === "Pago");
        setOrders(data);
      } catch (error) {
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-50">
      ...
      <div className="flex-1 overflow-y-auto">
         <div className="p-4 md:p-8">
            <h2 className="text-2xl font-bold text-slate-800 mb-6">Pedidos Longo Prazo (Encomendas)</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
               <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                 <div className="flex justify-between items-start">
                   <div>
                     <p className="text-sm text-slate-500 mb-1">Total a Importar/Solicitar</p>
               ...
               <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                 <div className="flex justify-between items-start">
                   <div>
                     <p className="text-sm text-slate-500 mb-1">Total de Lotes</p>
                     <p className="text-3xl font-bold text-slate-800">{orders.length}</p>
                   </div>
                   <div className="bg-blue-50 p-3 rounded-xl text-[var(--color-kora-blue)]"><Plane size={24} /></div>
                 </div>
               </div>
               <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                 <div className="flex justify-between items-start">
                   <div>
                     <p className="text-sm text-slate-500 mb-1">Clientes na Fila</p>
                     <p className="text-3xl font-bold text-slate-800">{new Set(orders.map(o => o.client.cpf)).size}</p>
                   </div>
                   <div className="bg-orange-50 p-3 rounded-xl text-orange-600"><Users size={24} /></div>
                 </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
               {loading ? (
                 <div className="p-8 text-center text-slate-500">Buscando encomendas...</div>
               ) : orders.length === 0 ? (
                 <div className="p-8 text-center text-slate-500">A fila de encomendas está vazia.</div>
               ) : (
                 <>
                     ...
                     <table className="w-full text-left border-collapse">
                       <thead>
                         <tr className="bg-slate-50 text-slate-500 text-sm border-b border-slate-100">
                           ...
                           <th className="p-4 font-medium">Itens Solicitados</th>
                           <th className="p-4 font-medium">Recebido em</th>
                           <th className="p-4 font-medium">Prazo Info</th>
                         ...
                         {orders.map((order) => (
                           <tr key={order.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                             <td className="p-4">
                               ...
                               <p className="text-xs text-slate-400">{order.client.cpf}</p>
                             </td>
                             <td className="p-4">
                             ...
                             <td className="p-4">
                                <span className="inline-flex items-center gap-1 bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-bold">
                                  <Clock size={12} /> 25 a 30 dias úteis
                                </span>
                             </td>
                      ...
                      {orders.map((order) => (
                         <div key={order.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col gap-3">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <span className="text-xs font-bold text-yellow-700">Prazo Kora: 25 a 30 dias úteis</span>
                            </div>
                            <button onClick={() => setSelectedOrder(order)} className="w-full mt-2 bg-blue-50 text-blue-600 hover:bg-blue-100 font-bold p-3 rounded-xl flex justify-center items-center gap-2 transition-colors">
                               <Eye size={18} /> Abrir Ficha de Encomenda
                            </button>
                         </div>
                      ))}
                   </div>
